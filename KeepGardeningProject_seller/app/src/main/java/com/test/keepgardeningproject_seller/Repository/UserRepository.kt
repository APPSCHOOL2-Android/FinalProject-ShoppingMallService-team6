...
class UserRepository {
    companion object{
        ...
        fun addUserInfo(userClass: UserClass, callback1:(Task<Void>) -> Unit){
            ...
            UserInfoRef.push().setValue(userClass).addOnCompleteListener(callback1)
        }
        ...
        fun getUserInfoByUserId(loginUserId:String, callback1: (Task<DataSnapshot>) -> Unit){
            ...
            UserInfoRef.orderByChild("userId").equalTo(loginUserId).get().addOnCompleteListener(callback1)
        }
        ...
        fun getUserInfoByUserIdx(userIdx:Long, callback1: (Task<DataSnapshot>) -> Unit){
            ...
            UserInfoRef.orderByChild("userIdx").equalTo(userIdx.toDouble()).get().addOnCompleteListener(callback1)
        }
        // 상점 정보가 있으면
